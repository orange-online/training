 ...
 * holder.
 */
package cn.deepdraw.training.framework.utils.collection;
...
public class MultiValueHashMap<K, V> extends AbstractMultiValueMap<K, V> implements Serializable {
     ...
     */
    public MultiValueHashMap(MultiValueMap<? extends K, ? extends V> map) {
        this();
        putAll(map);
    }
     ...
     * @param map the map
     */
    private <T extends K, U extends V> void putAll(MultiValueMap<T, U> map) {
     ...
     * @throws NullPointerException if the specified map is {@code null}
     */
    public MultiValueHashMap(Map<? extends K, ? extends V> map) {
        this();
