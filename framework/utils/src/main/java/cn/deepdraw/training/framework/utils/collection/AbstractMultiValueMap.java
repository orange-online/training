...
public abstract class AbstractMultiValueMap<K, V> implements MultiValueMap<K, V>, Serializable {
     ...
     *                              is {@code null}.
     */
    public AbstractMultiValueMap(Map<K, List<V>> store) {
        if (store == null) {
     ...
     */
    @Override
    public final void addAll(K key, @SuppressWarnings("unchecked") V... newValues) {
        if (newValues == null) {
    ...
    @Override
    public boolean equalsIgnoreValueOrder(MultiValueMap<K, V> omap) {
        if (this == omap) {
            return true;
        }
